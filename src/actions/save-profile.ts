...
export async function saveTrainingPreferences(trainingGoal?: string, trainingDaysPerWeek?: number, sessionDurationMinutes?: number) {
  const data: ProfileData = { trainingGoal, trainingDaysPerWeek, sessionDurationMinutes }
  const fieldsToUpdate: (keyof ProfileData)[] = ['trainingGoal', 'trainingDaysPerWeek', 'sessionDurationMinutes']
  return saveUserProfile(data, fieldsToUpdate)
}
...
function validateProfileData(data: ProfileData): string[] {
  ...
  if (data.trainingGoal && data.trainingGoal.trim() !== "" && !["strength", "muscle_gain", "fat_loss", "health"].includes(data.trainingGoal)) {
    errors.push("Invalid training goal selection")
  }

  // Training days validation - nur validieren wenn definiert und nicht leer
  if (data.trainingDaysPerWeek !== undefined && data.trainingDaysPerWeek !== null && data.trainingDaysPerWeek !== 0) {
  ...
  if (data.sessionDurationMinutes !== undefined && data.sessionDurationMinutes !== null && data.sessionDurationMinutes !== 0) {
