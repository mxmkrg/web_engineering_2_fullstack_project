...
function generateWorkoutDates(weeksBack: number): Date[] {
  ...
  for (let week = 0; week < weeksBack; week++) {
    ...
    for (const dayOffset of [0, 2, 4]) { // Monday, Wednesday, Friday
...
function calculateWeight(baseWeight: number, weekNumber: number, progressionRate: number): number {
...
function calculateReps(baseReps: number, weekNumber: number, isProgressing: boolean): number {
...
function generateWorkoutProgression(): any[] {
  ...
  const rotations = [
    ...
    ...Array(12).fill(null).map((_, i) => 
      i % 2 === 0 ? 'fullBodyA' : 'fullBodyB'
    ),
    // Weeks 5-8: Upper/Lower split
    ...Array(12).fill(null).map((_, i) => 
      i % 2 === 0 ? 'upperBody' : 'lowerBody'
    ),
      ...
      exercises: template.exercises.map(exercise => ({
        ...
        sets: Array(exercise.sets).fill(null).map((_, setIndex) => {
          const weight = exercise.baseWeight ? 
            calculateWeight(exercise.baseWeight, weekNumber, exercise.progressionRate) : 
            null;
          ...
          const finalReps = isLastSet && Math.random() > 0.7 ? 
...
export async function seedWorkouts() {
	try {
		...
		for (const workoutData of sampleWorkouts) {
			// Create the workout with archived status
			...
			for (let exerciseIndex = 0; exerciseIndex < workoutData.exercises.length; exerciseIndex++) {
				...
				for (let setIndex = 0; setIndex < exerciseData.sets.length; setIndex++) {
					...
					await db.insert(workoutSet).values({
						...
						completed: true, // Mark as completed since these are archived workouts
						notes: null,
					});
				}
			}
