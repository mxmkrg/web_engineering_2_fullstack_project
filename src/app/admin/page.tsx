...
export default function AdminPage() {
	...
	const handleSeedWorkouts = async () => {
		...
		try {
			...
			setWorkoutMessage(
				result.success ? result.message! : result.error || "Failed to seed workouts",
			);
		} catch (error) {
	...
	const handleClearWorkouts = async () => {
		...
		try {
			...
			setClearMessage(
				result.success ? result.message! : result.error || "Failed to clear workouts",
			);
		} catch (error) {
			...
			<div className="space-y-6">
				<div>
					...
					<p className="text-gray-600 text-sm mb-4">
						This will populate the database with a comprehensive exercise library (66 exercises total):
						<br />• Chest (8), Back (10), Legs (10), Shoulders (8) - major muscle groups
						<br />• Arms (6), Core (5), Cardio (6), Functional (3) - supporting exercises
						<br />Each exercise includes detailed instructions and muscle group targeting.
					</p>
				...
				<div>
					...
					<p className="text-gray-600 text-sm mb-4">
						This creates a complete 3-month beginner gym journey (36 workouts total):
						<br />• <strong>Weeks 1-4:</strong> Full body workouts building foundation strength
						<br />• <strong>Weeks 5-8:</strong> Upper/lower split for increased training volume
						<br />• <strong>Weeks 9-12:</strong> Push/pull/legs split for muscle specialization
						<br />• Realistic progressive overload with varying muscle group development
						<br />• Perfect for testing progression statistics and filtering features
					</p>
					...
					>
						{isSeedingWorkouts ? "Creating Journey..." : "Seed 3-Month Workout Journey"}
					</button>

					{workoutMessage && (
						<div
							className={`mt-3 p-3 rounded ${
								workoutMessage.includes("Error") || workoutMessage.includes("Failed")
									? "bg-red-50 text-red-600"
									: "bg-green-50 text-green-600"
							}`}
						>
				...
				<div>
					...
					<p className="text-gray-600 text-sm mb-4">
						This will permanently delete all workouts, exercises, and sets for the current user.
					...
					{clearMessage && (
						<div
							className={`mt-3 p-3 rounded ${
								clearMessage.includes("Error") || clearMessage.includes("Failed")
									? "bg-red-50 text-red-600"
									: "bg-green-50 text-green-600"
							}`}
						>
