    ...
    const sampleRoutines = [
      {
        ...
        exercises: [
          ...
          {
            pattern: ["shoulder", "overhead", "military"],
            sets: 3,
            reps: "10-12",
            weight: 95,
            rest: 90,
    ...
    await db.transaction(async (tx) => {
      for (const routineData of sampleRoutines) {
        ...
        const routineExists = existingRoutine.some(
          (r) => r.name === routineData.name,
        );

        if (routineExists) {
          console.log(
            `Routine "${routineData.name}" already exists, skipping...`,
          );
          continue;
        }
        ...
        for (let i = 0; i < routineData.exercises.length; i++) {
          ...
          const matchingExercises = getExercisesByPattern(
            exerciseConfig.pattern,
          );
